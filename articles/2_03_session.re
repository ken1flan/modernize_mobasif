...
@<href>{https://perldoc.jp/docs/modules/CGI-Session-3.11/Session.pod,CGI::Session}という便利なモジュールがあるので、基本的にそれを使っています。こちらではそれを適宜呼び出すのみです。



リクエストのクッキーを変数に復元した後に、そこに書かれたセッションを復元します。
復元のキーはクッキーに保存してある@<tt>{session_id}で、この値がなければ新しくセッションを作成し、セッションIDをクッキーに保存しています。
